...
public class LobbyUI : MonoBehaviour {
    ...
    private void Start() {
        ...
        playerNameInputField.onValueChanged.AddListener((string newValue) => {
            KitchenGameMultiplayer.Instance.SetPlayerName(playerNameInputField.text);
        });
    ...
    private bool ValidateJoinCodeInputs() {
        if (joinCodeInputField.text.Length == 0) {
            joinCodeErrorText.gameObject.SetActive(true);
            joinCodeErrorText.text = "\"Lobby Code\" cannot be empty";
            return false;
        }
        return true;
    }
